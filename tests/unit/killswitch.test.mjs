...
test("isHalted: global halt stops everything", () => {
  const r = isHalted({ global: true, scopes: [], reason: "incident" }, ["enterprise", "consulting", "client-a"]);
  assert.equal(r.halted, true);
  assert.match(r.reason, /global halt/i);
});

test("isHalted: exact scope match halts", () => {
  const r = isHalted({ global: false, scopes: ["client-a"] }, ["enterprise", "consulting", "client-a"]);
  assert.equal(r.halted, true);
  assert.match(r.reason, /client-a/);
});

test("isHalted: ancestor in the chain halts the descendant", () => {
  const r = isHalted({ global: false, scopes: ["consulting"] }, ["enterprise", "consulting", "client-a"]);
  assert.equal(r.halted, true);
  assert.match(r.reason, /consulting/);
});

test("isHalted: no match => not halted", () => {
  const r = isHalted({ global: false, scopes: ["client-b"] }, ["enterprise", "consulting", "client-a"]);
...
test("breaker: trips on the per-scope ceiling", () => {
  const now = new Date().toISOString();
  const recs = Array.from({ length: 4 }, () => ({ ts: now, scope: "client-a" }));
  const r = breaker({ limits: { windowMinutes: 10, maxRequestsGlobal: 100, maxRequestsPerScope: 4 } }, ledger(recs), "client-a");
  assert.equal(r.tripped, true);
