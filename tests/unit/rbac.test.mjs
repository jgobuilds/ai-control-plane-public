...
test("scope narrowing: approver must ALSO be in the scope's group", () => {
  const r = authorizedToApprove(
    {
      ...
      approverGroups: ["agents-approvers@example.com"], // ok for byRisk high, NOT in tenant-a byScope
      scope: "tenant-a",
      riskLevel: "high",
    },
    policy
  ...
  assert.equal(r.ok, false);
  assert.match(r.reason, /scope "tenant-a"/);
});
