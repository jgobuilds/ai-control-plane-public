...
test("resolveScope: known scope returns its ancestry chain", () => {
  const s = resolveScope("client-a");
  assert.deepEqual(s.chain, ["enterprise", "consulting", "client-a"]);
});

test("ingest: refuses a silo-isolated scope (must use its dedicated runner)", () => {
  // client-a is isolation:"silo" — its data must never land in the shared workspace.
  assert.throws(() => ingest("client-a", "notes.md", "some text"), /silo-isolated/);
});
