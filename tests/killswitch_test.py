...
CHAIN = {
    "tenant-a":   ["enterprise", "consulting", "tenant-a"],
...
with tempfile.TemporaryDirectory() as d:
    ...
    check("global halt: halts tenant-a", is_halted(st, CHAIN["tenant-a"]))
    check("global halt: halts jane", is_halted(st, CHAIN["jane"]))

    run(d, "resume", "global")
    st = read_state(d)
    check("resume global: nothing halted", not is_halted(st, CHAIN["tenant-a"]))

    # 2. scope halt stops just that scope
    run(d, "scope", "tenant-a", "leak review")
    st = read_state(d)
    check("scope halt: tenant-a listed", "tenant-a" in st["scopes"])
    check("scope halt: halts tenant-a", is_halted(st, CHAIN["tenant-a"]))
    ...
    check("ancestor halt: halts descendant tenant-a", is_halted(st, CHAIN["tenant-a"]))
