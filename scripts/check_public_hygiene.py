...
#
# An EXAMPLE file is not one of them, and that distinction cost a real finding.
# `context/brand-tokens.example.css` sat here and said "../brightside-instance"
# — an example is meant to be COPIED, so naming one org in it hands every reader
# our overlay path as the thing to imitate. Config that describes the local
# machine (.gitignore, .pre-commit-config.yaml) legitimately names it; a
# template must not.
ALLOW = {
