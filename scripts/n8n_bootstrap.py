...
stable ids (e.g. "brightworks-notify") make cross-workflow references (a Notify
...
workflow is `docker cp`'d in, then imported by the in-container `n8n import:workflow`.
  ...
  python scripts/n8n_bootstrap.py --dry-run brightworks-daily-digest   # execute once, no activate
  python scripts/n8n_bootstrap.py --activate brightworks-daily-digest
...
def set_active(container, wid, state):
    r = sh(["docker", "exec", container, "n8n", "update:workflow", f"--id={wid}", f"--active={state}"])
