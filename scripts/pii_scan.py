...
def in_tracked(root, needle):
    """Does this exact text also live in a TRACKED file?

    The quoted-from-ignored-file check has one systematic false positive: a
    gitignored BUILD ARTIFACT that is a byte copy of tracked source. A deploy
    tree, a rendered site, a bundled dist/ — quote your own published copy in a
    commit message and it reads as leaking an ignored file.

    If the same text is in a tracked file it is already in the repo by
    definition, so quoting it leaks nothing. Checked with `git grep` so it uses
    git's own index rather than a second walk of the tree, and only ever runs
    after a quote has already matched an ignored file — which is rare.

    Fails CLOSED: if git errors, we do not know, so we do not suppress.
    """
    if not needle.strip():
        return False
    r = git(root, "grep", "--fixed-strings", "--ignore-case", "--quiet",
            "-I", "-e", needle)
        ...
        for path in ignored_files(root):
            ...
            for q in quotes:
                if q.lower() in body.lower():
                    # Tracked content duplicated into an ignored artifact is not
                    # a leak — see in_tracked() for the build-copy case.
                    if in_tracked(root, q):
                        continue
