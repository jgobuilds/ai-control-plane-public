...
def sh(cmd):
    ...
    argv = ["wsl", "-e", "bash", "-lc", cmd] if IS_WIN else ["bash", "-lc", cmd]
...
def discover_mounts(name):
    ...
    for line in (r.stdout or "").splitlines():
        ...
        if len(p) == 3 and p[1]:
            src = p[0].replace("\\", "/")
            src = src.split("/brightworks/")[-1] if "/brightworks/" in src else os.path.basename(src)
...
def mermaid(services, live, available):
    ...
    L.append("  end")
    L.append('  subgraph NET["brightworks_agentnet — internal, no host port"]')
    for s in inte:
...
def render_html(services, active, inactive, repo, ts, mounts, schedules):
    ...
    H = ['<!doctype html><html lang="en"><head><meta charset="utf-8">',
         ...
         "<title>Brightworks — live architecture</title>",
         f"<style>{HTML_CSS}</style></head><body><div class='page'>"]
    H.append('<header><p class="eyebrow">Brightworks · live stack</p>'
    ...
    H.append('<section><h2>Trust boundary</h2>'
             ...
             '<div class="zone internal"><div class="zone-h"><span class="zone-n">Internal — brightworks_agentnet</span>'
