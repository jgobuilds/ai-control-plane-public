#!/usr/bin/env python3
"""Brightworks installer — interactive bootstrap for the big decisions.
...
TOKEN_KEYS = ["RUNNER_TOKEN", "ROUTER_TOKEN", "SCRUBBER_TOKEN"]
...
def main():
    ap = argparse.ArgumentParser(description="Brightworks interactive installer.")
    ...
    print(f"  Tokens: {'reusing '+', '.join(reused) if reused else 'generating 3 distinct secrets'}"
          + ("" if len(reused) == 3 else " (+generated the rest)"))
