...
// TWO CREDENTIALS, one per door, and they must not be the same secret.
//   ASK_TOKEN   presented to n8n's ask-human webhook (x-ask-human-token).
//   TOKEN       presented to this runner's OWN /ask/status over loopback
//               (x-runner-token), because that is the runner server's own auth.
// This used to send RUNNER_TOKEN to n8n as well, which meant n8n had to hold the
// runners' inbound credential to verify it. With env access in n8n nodes on, any
// workflow authored in the UI could then call a runner directly and skip the
// router (threat-model C3). n8n now holds only ASK_HUMAN_TOKEN, which no runner
// accepts.
const ASK_TOKEN = process.env.ASK_HUMAN_TOKEN || "";
...
const TOOL = {
  ...
  description:
    ...
    "fact you need is genuinely unavailable to you — a folder id, which tenant a " +
...
async function askHuman(args) {
  ...
  if (!ASK_TOKEN || !TOKEN) {
    const missing = [!ASK_TOKEN && "ASK_HUMAN_TOKEN", !TOKEN && "RUNNER_TOKEN"].filter(Boolean);
    return content(
      `ask_human is not configured: ${missing.join(" and ")} unset in this runner, so the ` +
      "question could not be raised or its answer read back. No human was asked.", true);
  }
  ...
  try {
    res = await fetch(ENDPOINT, {
      method: "POST",
      headers: { "content-type": "application/json", "x-ask-human-token": ASK_TOKEN },
      body,
