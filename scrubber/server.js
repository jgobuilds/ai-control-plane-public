...
// can't be automated — a human edits out tenant-recognizable specifics); commit
...
async function promotePreview(scope, file, toScope) {
  ...
  return { from: scope, to: toScope, file: path.basename(src), text: clean.text,
    ...
    note: "Review + generalize before commit: remove tenant-recognizable specifics the scrub can't detect (names of projects, unusual numbers, quotes)." };
}
