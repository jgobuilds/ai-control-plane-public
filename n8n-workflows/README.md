...
- **Human gate built in:** the batch goes through the approval gate before
  upload. `requireApproval: true` in Config (keep it on for client-facing or
...
with an outbound WebSocket but needs a WebSocket client, and `lanes` is
...
**up** the scope tree (client → practice, team → department, etc.):
...
2. The approval gate shows the reviewer the full scrubbed content inline.
   **Promotions are never auto-approved** — widening visibility is always a
   ...
   PII, not client-recognizable specifics (project names, unusual numbers).
...
5. **Encode learning** — a second, cheap routed task writes the incident's
   lesson to `skills/<category>-<id>.md` (no client names, no PII), so the next
...
human-invoked action when a client leaves, and it writes a deletion certificate to
`audit/deletions/`.
...
- **`action: "advise"`.** The lane may only *recommend* a re-pin. The mode system
  blocks it from acting, so a confused or compromised run cannot change what we run.
...
```bash
docker exec -e N8N_RUNNERS_BROKER_PORT=5699 n8n n8n execute --id=brightworks-research-watch
```
...
- **Mount `./eval` into the n8n container** (e.g. `./eval:/data/eval`) so the Write
  ...
  client content until eval_capture tokenizes it) — terminate the webhook inside
