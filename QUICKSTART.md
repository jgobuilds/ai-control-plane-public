...
> **§7 Before real client data** before any real engagement.
...
you ──► n8n (localhost:5678) ──/route──► router ──► claude-runner / gemini-runner ──► vendor CLIs
                                  ...
                                  └─► agentsview (localhost:8090, cost/session view)
```

| Service | Port | Purpose |
|---|---|---|
| `n8n` | `127.0.0.1:5678` | orchestration UI + workflows (your front door) |
| `agentsview` | `127.0.0.1:8090` | local cost/session dashboard |
...
cp .env.example .env
```
Edit `.env` and set **three different** random secrets (services refuse to start
without them):
```sh
# generate three: openssl rand -hex 32   (run it 3×, paste each)
...
### 2.4 Start the core
```sh
docker compose up -d          # router + runners + scrubber + n8n + agentsview
```
- n8n → http://localhost:5678  ·  agentsview → http://localhost:8090
...
3. **Import the consumers** (`deliverables`, `promote-skill`, `incident-responder`,
   ...
   - in every **HTTP Request** node, set the header token that matches the target:
     `/route` → `ROUTER_TOKEN`, `/run`+`/fanout` → `RUNNER_TOKEN`, `scrubber/*` →
     `SCRUBBER_TOKEN`;
...
| Halt one scope | `python scripts/halt.py scope client-a "reason"` |
...
| Offboard a client (proof) | `python scripts/offboard_scope.py client-a --confirm --operator you` |
...
## 7. Before real client data (safety checklist)
...
- [ ] For **confidential / competing-client** scopes: they are `isolation:"silo"`
      **and** you've cut over to per-scope runners (§6). **Until then, pool-mode
      isolation is advisory (cwd only) — do not co-locate competing clients**
      (threat-model **C1/C2**).
...
New-NetFirewallHyperVRule -Name "WSL-Brightworks-n8n" -DisplayName "WSL Brightworks n8n (5678)" `
  -Direction Inbound -VMCreatorId $vm -Protocol TCP -LocalPorts 5678 -Action Allow
New-NetFirewallHyperVRule -Name "WSL-Brightworks-agentsview" -DisplayName "WSL Brightworks agentsview (8090)" `
  -Direction Inbound -VMCreatorId $vm -Protocol TCP -LocalPorts 8090 -Action Allow
```
...
- **Performance:** bind-mounts off `/mnt/<drive>` are slower than the native WSL
  filesystem. For heavier use, clone the repo *inside* WSL (e.g. `~/brightworks`).
