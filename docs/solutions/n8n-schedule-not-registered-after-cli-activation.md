...
`brightworks-dispatch` was activated with `n8n update:workflow --active=true`. `n8n list:workflow --active=true` listed it, and n8n's own database showed `active = 1`. Fifteen minutes and forty polls later it had produced **zero executions** on a ten-minute schedule.
