...
Every agent gets five things written down. Template:
...
| **Off-ramp** | What it does when it **lacks grounding** — the explicit "I don't know" path. Which fact it names as missing, who it escalates to, and what it must *not* emit instead. A persona without this field has only one exit, so the model takes it. |

**The off-ramp is a field because the failure it prevents is invisible.** An
agent with no defined "I don't know" does not fall silent; it produces
confidently-wrong output in the house format, at the usual length, with the
usual confidence. Nothing downstream distinguishes that from a good answer —
**a verifier checks that work is well-formed, not that its premise was true** —
so the refusal has to be available at the persona layer or it does not exist.
Write what the agent should *say*, not merely that it may decline: "names the
missing fact and escalates to X" is checkable; "knows its limits" is not.
...
## 7. How the control plane implements this
