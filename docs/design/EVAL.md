...
Brightworks already logs, per decision, a tamper-evident metadata record. This lane
...
- It is **complementary to — not a replacement for** — semantic / ground-truth
  ...
  through `/route` and grades the actual output (exact/contains/regex/LLM-as-judge),
...
side** of that metric (verifier pass + change-failure); `agentsview` supplies the
