...
Only detection is delegated; tokenization/rehydration stay in Brightworks, so the
...
- **Cloud DLP / Lakera / Bedrock:** front the managed API with a tiny adapter that
  emits one of the shapes above (keeps provider credentials out of Brightworks and
