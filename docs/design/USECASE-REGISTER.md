...
the control plane knows, at any moment, **every AI use case it runs, who owns it, how
...
"tenant-a": {
  ...
  "runnerHost": "claude-runner-tenant-a",
  "owner":   "owner-tenant-a@example.com",       // accountable human — a real
                                                 // address in YOUR config; an
                                                 // @example.com one in samples,
                                                 // because an invented name reads
                                                 // as a real person
  "useCase": "Tenant-A advisory & code-delivery agent",
...
review, not a glance. the control plane maps each direction onto an enforceable
