...
5. **Rebuild/restart** the runner as a canary, from the live checkout:
   `python scripts/rollout.py canary --probe`, then `observe` and `promote`
   ([UPGRADES.md](UPGRADES.md)). A bare `docker compose up -d --build` recreates
   every runner at once with nothing checked in between.
