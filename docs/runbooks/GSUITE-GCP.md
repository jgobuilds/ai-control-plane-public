...
| Tenant (consulting) | Tenant Shared Drive, membership = engagement staff | Engagement Google Group |
...
- **Drive labels** (Workspace data classification) mark documents `pii`,
  `tenant-confidential`, etc.; the sync workflow reads labels and routes
...
| Audit trail | Router `decision` blocks + **Cloud Audit Logs** shipped to **BigQuery**; dashboard in Looker Studio. `scripts/gen_status.py` stays as the local/dev lens. |
...
- **gemini-runner** gets a first-class home: authenticate with a **Vertex AI
  ...
  data inside your GCP project and VPC-SC perimeter, which is what tenant
  confidentiality actually requires. (The consumer free tier is fine for the
  personal/dev scope lane; use Vertex for tenant/department lanes.)
...
4. Silo runners for walled tenants as separate Cloud Run services w/ per-service
