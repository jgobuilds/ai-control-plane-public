...
## D. Before any of this touches client data

Not optional, and not in this order by accident:

1. **Consent** is settled in the engagement contract (ADR 0015). The client's
...
3. **The scrubber is proven on real input.** The vault exists and has never
   carried live data; the first client transcript is not the run to discover a
   defect on.
