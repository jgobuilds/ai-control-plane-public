# Testing — how to prove the control plane is secure and works as designed
...
| `scripts/mount_drift_check.py` | mount drift | A slot file (`CLAUDE.md`/`skills`/`glossary.md`) that exists on disk but no serving runner mounts; a compose mount whose host path is absent (docker would create an empty dir); with `--live`, a path in the generated compose that is missing from the RUNNING container — regenerated and never recreated. Conformance asserts mounts aren't too broad; this asserts they aren't too narrow. |
| `tests/mount_drift_test.py` | mount drift (self-test) | Synthesises a slot file added after generation and asserts the check goes red, then green once removed. Exists because the check reports on files that exist — a tree with no context files makes it pass while looking at nothing. |
| `tests/eval_analysis_test.py` | eval analysis | Exact-McNemar values by hand (a normal approximation disagrees at these counts); judge-agreement confidence (majority decides, a divided panel never reports 1.0, an even split FAILS, a single judge reports null not 1.0); resample stability (majority verdict, a flapping case never reports 1.0, a deterministic one never reports false variance); the results ledger and drift (a steady suite flags nothing, a real drop flags and exits non-zero, and a FLAT pass rate still surfaces per-case churn). |
| `tests/unit/router.test.mjs` | router (Node) | `tokenOk` (constant-time, empty ⇒ fail-closed unless `ALLOW_NO_AUTH`); `classifyRisk` matrix (silo·advise→high, warn·write→medium, apply·proactive→critical, blockCrossVendor at data ≥ 3); `resolveMode` (scope floor, request can only tighten, human-only caps advise); `clampTier`; `escalateForVendor` (a blocked scope escalates to the cheapest permitted tier, never downward, and never to a tier `targetFor` would then refuse); the audit hash-chain matches `verify_audit.py`. |
...
| `scripts/n8n_doctor.py` | n8n live state (Docker) | Faults no repo check can see, because they live in n8n's own database: an **orphaned webhook registration** whose owning workflow no longer exists (squats the path, survives a restart, invisible in the UI); two workflows claiming one path; and repo webhook paths n8n does not actually serve. Read-only — it prints the remedy rather than writing to live orchestration state. |
