...
give a provable client-offboarding story. Both are **stdlib-only**, deterministic,
...
> Silo scopes (e.g. `client-a`, `client-b`) keep their workspace + vault on their
...
`offboard_scope.py` is the right-to-deletion / client-exit tool. It removes:
...
```
python scripts/offboard_scope.py client-a                       # DRY-RUN
python scripts/offboard_scope.py client-a --confirm --operator jane
...
hand it to the departing client as proof, or keep it for a DSAR/audit response.
...
action (a client leaving), always run with `--confirm` by an operator.
