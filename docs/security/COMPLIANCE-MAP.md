...
the control plane maps **every governance control** (the [GOVERNANCE.md](GOVERNANCE.md)
...
## The honest gap list — what the control plane does NOT yet cover

Coverage is deliberately three-valued (`full` / `partial` / `none`). The gaps
below are cross-referenced to the **E2 / E3 roadmap** in
`MARKET-ANALYSIS.md` (internal) and the [THREAT-MODEL.md](THREAT-MODEL.md)
...
1. **Model-quality assurance** (eval, drift, bias, accuracy metrics, model cards)
   — the control plane governs *how an agent is allowed to act*, not *how good the
...
2. **Detection depth & identity** — heuristic guardrails and regex PII (E3), the
   ...
   strongest coverage is where the control plane is structural and unit-tested:
