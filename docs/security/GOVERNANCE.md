...
What governs an agent request in the control plane, and how each control is *enforced*
...
| 3b | **Tool allowlisting** | `scopes.json controls.allowedTools` | Router passes the per-scope tool list; the runner enforces it with `--tools` + `--disallowedTools mcp__<server>` (`--allowedTools` restricted nothing under skip-permissions until 2026-09-17 — threat-model H5) | scope config + runner arg |
...
| 13 | **Quality regression / improvement loop** (observability) | `scripts/eval_run.py` + `scripts/eval_capture.py` + `eval/datasets/**` + `n8n-workflows/feedback.workflow.json` | Offline **semantic/ground-truth** eval — the quality-regression counterpart to #12: prod failure → feedback flag → `eval_capture` (prompt tokenized via the scrubber) → `eval_run` replays each case through `/route` and grades it (exact/contains/regex/LLM-as-judge) → **exit non-zero on any regression**. The gate runs **host-side**, where the per-scope datasets live; CI proves the grader only (corrected 2026-09-24 — this said "CI quality gate" while nothing ran it; ADR 0020). Governed observability: eval data is per-scope, PII-tokenized, retention-bound, local — never cross-scope, never a third-party processor. Not a runtime enforcement gate — a pre-ship quality gate + governed eval store | `eval_run_test.py`, `eval/datasets/starter/`, see IMPROVEMENT-LOOP.md |
| 14 | **Container resource ceilings** | `docker-compose.yml` + `compose.scopes.yml` (generated by `scripts/gen_compose.py`) | Every service **and every per-tenant silo runner** declares `mem_limit`, `memswap_limit`, `cpus` and `pids_limit`. Closes threat-model **M7** ("a verify fork-bomb takes the host"): `pids_limit` is the control that actually stops it, and `memswap_limit == mem_limit` prevents a container at its ceiling escaping into host swap. Complements #0, which bounds request *rate* but cannot stop a single request exhausting the host. Sized from measured idle use with 1–2 orders of magnitude of headroom — a runner OOM-killed by its own guardrail turns the guardrail into the outage | `security_test.py` asserts all four **per service across both compose files**; verified live with `docker inspect`, not only in YAML |
