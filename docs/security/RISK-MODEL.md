...
| **autonomy** | request `trigger` | `turn` (human-initiated) | `scheduled` | `proactive` (self-triggered) |
...
| **blockCrossVendor** | data ≥ 3 | the verifier stays on the **same** vendor — confidential work is never sent to a second provider (addresses threat-model M4) |
...
{ "prompt": "...", "scope": "client-a",
  ...
  "trigger": "proactive",   // turn | scheduled | proactive
...
|---|---|---|
| client advisory read | **high** | isolation, no-cross-vendor |
| internal code-edit (turn) | medium | verify |
| incident apply (proactive) | **critical** | verify, approval |
| client apply (proactive) | **critical** | verify, approval, isolation, no-cross-vendor |
...
dependency-bumps → verify but no pre-gate because the gate is the merge). The
