...
| **Mem0** — Apache-2.0 | $0 licence | **Reject as a graph.** Mem0 **removed its own graph** after its paper showed it lost on single- and multi-hop recall, ran **3× slower** and cost **2× the tokens**. Disconfirming evidence from the vendor's own research is worth more than any claim |
...
**The strongest signal in the 2026 evidence is a negative one.** Mem0 deleting its
own graph after its own benchmarks is the kind of disconfirming finding that
should outweigh a dozen enthusiastic comparisons. The consensus is *hybrid* —
vector for recall, graph only where multi-hop reasoning and provenance genuinely
pay — not graph-by-default.
