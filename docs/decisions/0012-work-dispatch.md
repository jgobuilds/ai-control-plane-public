...
7. **GitHub Issues remains dogfooding-only.** ADR 0010 option H rejects
   bring-your-own trackers for *client* work — client-linked metadata to a third
   ...
   remembered: client work uses `file` or the router, never `github`.
...
**Decided, built and ACTIVE** — `brightworks-dispatch`, every 10 minutes. Verified by
